...
{
    [Fact]
    public async Task SeedAsync_ShouldNotCreate_WhenAllCategoriesExist()
    {
        var existing = Enum.GetValues<PRS.Model.Enums.Category>().Select(c => c.ToString()).ToArray();
        var cursor = new ForEachCursor(existing);

        var txMock = new Mock<IAsyncTransaction>();

        txMock.Setup(t => t.RunAsync("MATCH (c:Category) RETURN c.name AS name"))
              .ReturnsAsync(cursor);
    ...
    [Fact]
    public async Task SeedAsync_ShouldCreate_WhenNoCategoriesExist()
    {
