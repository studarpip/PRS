...
{
    [Fact]
    public async Task SeedAsync_ShouldNotCreate_WhenAllCountriesExist()
    {
        var existing = Enum.GetValues<Country>().Select(c => c.ToString()).ToArray();
        var cursor = new ForEachCursor(existing);

        var txMock = new Mock<IAsyncTransaction>();

        txMock.Setup(t => t.RunAsync("MATCH (c:Country) RETURN c.name AS name"))
              .ReturnsAsync(cursor);

        txMock.Setup(t => t.RunAsync(It.Is<string>(q => q.TrimStart().StartsWith("MERGE")), It.IsAny<object>()))
              .ReturnsAsync(Mock.Of<IResultCursor>());

        var seeder = new CountrySeeder();
    ...
    [Fact]
    public async Task SeedAsync_ShouldCreate_WhenNoCountriesExist()
    {
